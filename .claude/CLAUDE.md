...
| Frontend / UI work | `frontend-engineer` | ui-craft → verification-before-completion |
| Screenshot / image reference | `frontend-engineer` | image-to-design-spec → ui-craft |
| Infra / IaC / deploy | `devops-engineer` | dev-craft → devops-automation → verification-before-completion |
...
| Code review | `code-reviewer` | code-review-and-quality → verification-before-completion |
| Security audit | `security-auditor` | bug-hunting → verification-before-completion |
| Claim "done" | `verifier` | verification-before-completion (mandatory) |
...
|-------|-------|--------|------------|
| `triage` | deepseek-v4-flash-free | routing | prompt-optimizer, agent-router |
| `planner` | deepseek-v4-flash-free | planning | prompt-optimizer, planning-and-task-breakdown, grilling |
| `implementer` | big-pickle | implementation | prompt-optimizer, dev-craft, testing-strategies |
| `debugger` | deepseek-v4-flash-free | debugging | prompt-optimizer, debugging-and-error-recovery |
| `code-reviewer` | big-pickle | review | prompt-optimizer, code-review-and-quality |
| `verifier` | deepseek-v4-flash-free | verification | prompt-optimizer, verification-before-completion |
| `frontend-engineer` | big-pickle | frontend | prompt-optimizer, ui-craft, dev-craft |
| `api-designer` | deepseek-v4-flash-free | api-design | prompt-optimizer, api-design |
| `database-engineer` | deepseek-v4-flash-free | data | prompt-optimizer, dev-craft, database-migrations |
| `devops-engineer` | deepseek-v4-flash-free | infrastructure | prompt-optimizer, devops-automation |
| `security-auditor` | deepseek-v4-flash-free | security | prompt-optimizer, bug-hunting |
| `test-engineer` | big-pickle | testing | prompt-optimizer, testing-strategies |
| `docs-engineer` | big-pickle | documentation | prompt-optimizer, documentation-engineering |
| `retro-analyst` | deepseek-v4-flash-free | analysis | prompt-optimizer, retro, learn |

### Skill Chains (All start with `prompt-optimizer`)

```
Feature (vague):    prompt-optimizer → product-thinking → planning-and-task-breakdown → grilling → dev-craft
Feature (specs):    prompt-optimizer → project-discovery → planning-and-task-breakdown → grilling → dev-craft
Bug fix:            prompt-optimizer → debugging-and-error-recovery → implementer → verification-before-completion
Code review:        prompt-optimizer → code-review-and-quality → verification-before-completion → verification-before-completion
Security audit:     prompt-optimizer → bug-hunting → code-review-and-quality → verification-before-completion → verification-before-completion
Deployment:         prompt-optimizer → ship → verification-before-completion → verification-before-completion → learn
Retrospective:      prompt-optimizer → retro → learn
```

### Cost Optimization
- **prompt-optimizer** runs at triage (pre-routing) and per-agent → 20-40% token reduction
