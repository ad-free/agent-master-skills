...
When invoked by `triage` or `agent-router` before classification:
- Input: Raw user request
- Output: Optimized, structured request that triage can classify accurately
...
Each agent can declare its prompt optimization needs:
...
Profiles are loaded by the shared wrapper utility (`skills/references/prompt-optimizer-wrapper.md`).