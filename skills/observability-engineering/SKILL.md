...
dev-craft's HARDEN phase should invoke this skill to define the observability contract (logs/metrics/traces/SLOs) for the thing it just built, rather than adding logging/monitoring as an afterthought. bug-hunting covers attack surface; this covers visibility into behavior — they're separate concerns.
...
6. **Where does it go?**
   - Logs → Loki/Elastic/CloudWatch
   - Metrics → Prometheus/Thanos/Cortex/Mimir or managed (Grafana Cloud, Datadog)
   - Traces → Tempo/Jaeger/Zipkin or managed
   - Dashboards → Grafana (code-defined, not click-ops). → `reference/dashboard-as-code.md`

## Output

An observability contract doc per service: SLOs with error budgets, alert rules with owners/runbook links, log/metric/trace schema, and dashboard definitions — handed to dev-craft HARDEN.