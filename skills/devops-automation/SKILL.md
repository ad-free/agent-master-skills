...
description: Use when deciding deployment mechanics, rollback strategy, CI/CD pipeline shape, or infrastructure-as-code choices. Do NOT use for "is this ready to deploy" (see quality-gates) or for writing the pipeline YAML itself (that's plain BUILD work once the strategy is decided).
metadata:
...
1. **Where does this run?**
   - Kubernetes (EKS/GKE/AKS/self-managed) → ArgoCD (GitOps) + Argo Rollouts for progressive delivery. → `reference/argocd-rollouts.md`
